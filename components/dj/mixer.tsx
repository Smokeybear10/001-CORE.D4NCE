    ...
    <div className="rounded-xl bg-[#150535] border border-violet-500/[0.08] p-3 flex flex-col gap-3">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-1.5">
          <span className="text-[12px] font-mono font-semibold text-amber-400/80 tabular-nums">{bpmA ?? "—"}</span>
          {camelotA && (
            <span className="text-[9px] font-mono text-amber-400/50 bg-amber-500/10 px-1 rounded">{camelotA}</span>
          )}
        </div>
        <div className="flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(["A", "B"] as const).map((d) => (
                    <div key={d} className="space-y-1">
                      <span className={cn("text-[9px] font-mono", d === "A" ? "text-amber-400/60" : "text-cyan-400/60")}>{d}</span>
                      <Slider
              ...
              <div key={d} className="space-y-2">
                <div className="flex items-center gap-2">
                  <span className={cn(
                    ...
                    d === "A" ? "bg-amber-500/15 text-amber-400" : "bg-cyan-500/15 text-cyan-400"
                  )}>
                    Deck {d}
                  </span>
                </div>
